...
use App\Enums\Configs\BilmaConfigs;
...
if (!function_exists("paystackServiceFee")) {
	 ...
	 * @return float
	 */
	function paystackServiceFee(float $amount, bool $cape = true)
	{
		/**
		 * Service Table
		 * -------------
...
if (!function_exists("paystackCharges")) {
	/**
	 ...
	 * @return float
	 */
	function paystackCharges(float $amount)
	{
		/* get the service fee */
		$serviceFee = paystackServiceFee($amount);
...
if (!function_exists("paystackTransferCharges")) {
	/**
	 ...
	 * @return float
	 */
	function paystackTransferCharges(float $amount)
	{
		if ($amount <= 5000) {
			return 10;
		}

		if ($amount >= 5001 && $amount <= 50000) {
			return 25;
		}
...
if (!function_exists("formatPhoneNumber")) {
	/**
	 * Get an international formatted phone number representation fo the
	 ...
	 * @return string
	 */
	function formatPhoneNumber(string $phoneNumber, string $countryCode = "NG")
	{
		$phoneNumberUtil = PhoneNumberUtil::getInstance();
		$numbInfo = $phoneNumberUtil->parse($phoneNumber, $countryCode);

		return $phoneNumberUtil->format($numbInfo, PhoneNumberFormat::E164);
	}
}
...
if (!function_exists("isValidPhoneNumber")) {
	 ...
	 * @return bool
	 */
	function isValidPhoneNumber(string $phoneNumber, string $countryCode = "NG")
	{
		$phoneNumberUtil = PhoneNumberUtil::getInstance();
		$numbInfo = $phoneNumberUtil->parse($phoneNumber, $countryCode);

		if (!$phoneNumberUtil->isValidNumber($numbInfo)) {
			return false;
		}
...
if (!function_exists("splitAndFormatNumber")) {
	/**
	 * Splite a number and format it with an hyphen base on the splite count.
	 *
	 * @param string $number
	 ...
	 * @return string
	 */
	function splitAndFormatNumber(string $number, int $splitCount = 3)
	{
		$formattedNumber = "";
		$numberStr = strval($number);
		$groupCount = ceil(strlen($numberStr) / $splitCount);

		for ($i = 0; $i < $groupCount; $i++) {
			$start = max(strlen($numberStr) - ($i + 1) * $splitCount, 0);
			$length = min($splitCount, strlen($numberStr) - $start);
			$group = substr($numberStr, $start, $length);

			if (!empty($formattedNumber)) {
				$formattedNumber = $group . "-" . $formattedNumber;
			} else {
...
if (!function_exists("isSufficientBalance")) {
	/**
	 * Check if user has sufficient balance
	 *
	 * @param int|float $mainBalance
	 * @param int|float $transactionAmount
	 * @param int|float $monthlyCharge
	 *
	 * @return bool
	 */
	function isSufficientBalance(int|float $mainBalance, int|float $transactionAmount, int|float $monthlyCharge)
	{
		return $mainBalance > 0 && $mainBalance >= $transactionAmount && $mainBalance - $transactionAmount >= $monthlyCharge;
	}
}

if (!function_exists("hasSufficientPointsBalance")) {
	/**
	 * Check if the points balance is sufficient.
	 *
	 * @param int|float $pointsBalance
	 * @param int|float $amount
	 *
	 * @return bool
	 */
	function hasSufficientPointsBalance(int|float $pointsBalance, int|float $amount)
	{
		if ($pointsBalance <= 0) {
			return false;
		}

		$amount = convertToPositiveInteger($amount);

		$pointsToMonetaryRate = (float) BilmaConfigs::POINTS_CONV_RATES->value;
		$pointsMonetaryBalance = $pointsBalance / $pointsToMonetaryRate;

		return $amount <= $pointsMonetaryBalance;
	}
}

if (!function_exists("convertToPositiveInteger")) {
	/**
	 * Convert a value to a postive integer if the provided value is an integer value.
	 *
	 * @param mixed $value
	 *
	 * @return mixed
	 */
	function convertToPositiveInteger(string $value)
	{
		...
		if (is_int($value) || is_float($value)) {
			...
			return $value; // or any other appropriate action
		}
	}
}

if (!function_exists("getMeterProviders")) {
	function getMeterProviders(?string $key = null)
	{
		$providers = [
			"aba-electric" => "ABEDC - Aba Electricity",
			"ikeja-electric" => "IKEDC - Ikeja Electricity",
			"eko-electric" => "EKEDC - Eko Electricity",
			"kano-electric" => "KEDCO - Kano Electricity",
			"portharcourt-electric" => "PHED - Port Harcourt Electricity",
			"jos-electric" => "JED - Jos Electricity",
			"ibadan-electric" => "IBEDC - Ibadan Electricity",
			"kaduna-electric" => "KAEDCO - Kaduna Electricity",
			"abuja-electric" => "AEDC - Abuja Electricity",
			"enugu-electric" => "EEDC - Enugu Electricity",
